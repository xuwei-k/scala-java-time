...
import scalajs.testsuite.utils.AssertThrows._
...
class LocaleBuilderTest {
  ...
  @Test def test_build_with_extensions(): Unit = {
    ...
    // null resets
    val b2 = new Locale.Builder()
    val locale2 = b2.setExtension('a', "ca-japanese").setExtension('a', "").build
  ...
  @Test def test_build_with_unicode_extensions(): Unit = {
    val b1 = new Locale.Builder()
    val locale = b1.setUnicodeLocaleKeyword("nu", "thai").setUnicodeLocaleKeyword("ok", "").build
    ...
    assertTrue(locale.getExtensionKeys.contains(Locale.UNICODE_LOCALE_EXTENSION))
  ...
  @Test def test_replace_unicode_extensions(): Unit = {
    val b1 = new Locale.Builder()
    val locale1 = b1.setUnicodeLocaleKeyword("nu", "thai").setUnicodeLocaleKeyword("ok", "").build
    assertEquals("thai", locale1.getUnicodeLocaleType("nu"))
    val locale2 = b1.setExtension(Locale.UNICODE_LOCALE_EXTENSION, "newvalue").build
    assertTrue(locale2.getUnicodeLocaleKeys.isEmpty)
    assertNull(locale2.getUnicodeLocaleType("nu"))

    // Check that the extensions is propagated to the general extension
    assertTrue(locale2.getExtensionKeys.contains(Locale.UNICODE_LOCALE_EXTENSION))
  ...
  @Test def test_remove_unicode_attribute(): Unit = {
    val b1 = new Locale.Builder()
    val locale1 = b1.addUnicodeLocaleAttribute("attr").removeUnicodeLocaleAttribute("attr").build
  ...
  @Test def test_clear_extensions():Unit = {
    val b = new Locale.Builder()
    val locale1 = b.setLanguage("es").setRegion("CL").setExtension('a', "ca-japanese").build
